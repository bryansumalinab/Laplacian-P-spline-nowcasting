...
nowcast.date <- as.Date("2021-06-30") #set nowcast date
...
######## (a) CI COVERAGE #######
COVERAGE_June_f12 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_June_f12

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_June_f12 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_June_f12

######## (c) SMAPE #######
SMAPE_June_f12 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_June_f12
