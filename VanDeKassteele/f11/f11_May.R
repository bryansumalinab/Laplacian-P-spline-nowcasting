...
nowcast.date <- as.Date("2021-05-31") #set nowcast date
...
######## (a) CI COVERAGE #######
COVERAGE_May_f11 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_May_f11

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_May_f11 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_May_f11

######## (c) SMAPE #######
SMAPE_May_f11 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_May_f11
