Nowcasting <- function(data, # Data of cases for each delay and date of occurence
                       date.now, # Nowcast date
                       Kt = 40, # Number of B-splines for time dimension
                       Kd = 10, # Number of B-splines for delay dimension 
                       v_init = c(1,1) # Initial values for log-penalty parameter
  ...
  # Precision for day of the week effects coefficient
  zeta <- 1e-05
  
  D <- max(data$d) # Maximum delay
  T <- max(data$t) # Maximum time
  
  # Rows for not yet reported
  nyr <- which(data$Reported == "Not yet reported")
  
  # Data for not yet reoprted (used for estimation)
  data_nyr <- data[nyr,]
  ...
  Kt <- dim(Bt)[2] # Dimension of B-spline matrix for time
  Kd <- dim(Bd)[2] # Dimension of B-spline matrix for delay
  p <- ncol(Z)-1   # Day of the week coefficients
