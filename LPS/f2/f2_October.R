...
######## (a) CI COVERAGE #######
COVERAGE_October_f2 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_October_f2

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_October_f2 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_October_f2

######## (c) SMAPE #######
SMAPE_October_f2 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_October_f2
