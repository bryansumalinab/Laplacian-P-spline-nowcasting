...
######## (a) CI COVERAGE #######
COVERAGE_April_f2 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_April_f2

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_April_f2 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_April_f2

######## (c) SMAPE #######
SMAPE_April_f2 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_April_f2
