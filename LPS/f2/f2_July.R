...
######## (a) CI COVERAGE #######
COVERAGE_July_f2 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_July_f2

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_July_f2 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_July_f2

######## (c) SMAPE #######
SMAPE_July_f2 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_July_f2
