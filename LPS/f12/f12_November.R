...
date.now <- as.Date('2021-11-30') #set nowcast date
...
######## (a) CI COVERAGE #######
COVERAGE_November_f12 <- round(apply(CI, MARGIN = 1, mean,na.rm = T)*100,2)
COVERAGE_November_f12

######## (b) MAPE #######
#replace infinity with NA
mape[sapply(mape, is.infinite)] <- NA
MAPE_November_f12 <- round(apply(mape, MARGIN = 1, mean,na.rm = T)*100,2)
MAPE_November_f12

######## (c) SMAPE #######
SMAPE_November_f12 <- round(apply(smape, MARGIN = 1, mean,na.rm = T)*100,2)
SMAPE_November_f12
