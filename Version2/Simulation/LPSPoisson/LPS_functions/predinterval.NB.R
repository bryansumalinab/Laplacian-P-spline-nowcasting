predinterval.NB <- function(data, # data
  ...
  # Covariance of xi
  mu.nb.nyr <- function(xi) exp(as.numeric(X_nyr %*% xi))
  var.nb.nyr <- function(xi, v) mu.nb.nyr(xi) + (1/exp(v[3]))*(mu.nb.nyr(xi)^2)
  W.nb.nyr <- function(xi, v) diag(((exp(as.numeric(X_nyr %*% xi)))^2)*(1/var.nb.nyr(xi, v)))
  M.nb.nyr <- function(xi) diag(y - mu.nb.nyr(xi))
  V.nb.nyr <- function(xi, v) diag(mu.nb.nyr(xi) * (1/var.nb.nyr(xi, v) - 
                                                      (mu.nb.nyr(xi)/(var.nb.nyr(xi, v)^2)) * 
                                                      (1 + 2*mu.nb.nyr(xi)*(1/exp(v[3])))))
  Qv.nyr <- Qv(v_mode[1:2])
  sigma_xi <- solve(-t(X_nyr)%*%M.nb.nyr(xi = xi_mode)%*%V.nb.nyr(xi = xi_mode, v = v_mode)%*%X_nyr + 
                      t(X_nyr)%*%W.nb.nyr(xi = xi_mode, v = v_mode)%*%X_nyr + Qv.nyr) # covariance for xi
  ...
  for (i in 1:dim(data_CI)[1]) {
    ...
    data_CI[i,"lower_nyr"] <- quantile(r.nb.sum,probs = 0.025)
    data_CI[i,"upper_nyr"] <- quantile(r.nb.sum,probs = 0.975)
  }
